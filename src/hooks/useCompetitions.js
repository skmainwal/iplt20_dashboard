...
const useCompetitions = () => {
  ...
  const fetchCompetitions = async (category) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/competitions/${categoryLower}`);

      if (response.data.success) {
        if (categoryLower === 'men') {
          setMensCompetitions(response.data.data);
          if (selectedCategory === 'MEN' && !selectedCompetition) {
            const defaultCompetition = response.data.data.find(comp => comp.isLive) || response.data.data[0];
  ...
  const handleCategoryChange = (category) => {
    ...
    const defaultCompetition = competitions.find(comp => comp.isLive) || competitions[0];
    setSelectedCompetition(defaultCompetition);
  };
