...
const useTopRunScorers = (competitionId = "148") => {
    ...
    const fetchTopRunScorers = async() => {
        try {
            ...
            const response = await api.get(`/api/top-run-scorers/${competitionId}`);

            if (response.data.success) {
                setTopRunners(response.data.data);
            } else {
                throw new Error(
                    response.data.message || "Failed to fetch top run scorers"
                );
            }
        } catch (err) {
    ...
    // Sort functions
    const sortByRuns = () => {
        const sorted = [...topRunners].sort(
            (a, b) => parseInt(b.TotalRuns) - parseInt(a.TotalRuns)
        );
